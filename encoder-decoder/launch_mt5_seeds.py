...
# -*- coding: utf-8 -*-
"""
Launches t5_multi_finetune_context_mapped_seeds.py in parallel,
one process per seed, each pinned to a dedicated GPU.

Usage:
    python launch_mt5_seeds.py
"""
...
SCRIPT = os.path.join(os.path.dirname(__file__),
                      't5_multi_finetune_context_mapped_seeds.py')
PYTHON = sys.executable   # same interpreter / conda env that runs this launcher

# seed → GPU index (adjust GPU assignments as needed)
RUNS = [
    {"seed": 123, "gpu": 0},
    {"seed": 456, "gpu": 1},
]
...
for run in RUNS:
    ...
    log_path = f"logs/mt5_run_seed{seed}_gpu{gpu}.log"
...
print(f"\nAll {len(RUNS)} runs started. Waiting for completion ...\n")

...
corpus_mapped = 'corpus_mapped_en'
df_train_mapped = pd.read_csv(f"{corpus_mapped}/all_en.tsv", sep='\t', dtype=str)

df_all = pd.concat([df_tr, df_d, df_train_mapped])

n = len(df_all) // 10
df_train = df_all.iloc[n:]
df_dev = df_all.iloc[:n]
...
def prepare_train_data(data_df):
    data_df['input_context'] = data_df['instruction'] + ' ' + data_df['input_with_entity'] + '; context: ' + data_df['context']
...
output_model=output_dir = f"./output_codiesp/{model_id.split('/')[-1]}-system-instruct-context-mapped-en"
